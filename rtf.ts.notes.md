...
- Similarly to `useAppSelect`, we create a custom hook to ensure type-safety, but this time to MODIFY DATA, the `useAppDispatch`
...
/**
 * Use this hook to dispatch actions
 */
export const useAppDispatch: () => AppDispatch = useDispatch;

/**
 * Use this hook to select data from the Redux store
 */
export const useAppSelector: TypedUseSelectorHook<RootState> = useSelector;
```
...
- RTK Includes `createAsyncThunk` for handling async logic (API calls)
...
const initialState: DataState = { items: [], loading: false };
...
- Handles async state operations (`pending`, `fulfilled`, `rejected`)
- Ensures type inference

### reducers vs. extraReducers

#### reducers (local action)

- Defines `actions and reducers` that belong to the slice itself
- Actions are auto-generated by RTK
- used for **sync state updates**

#### extraReducers (external actions)

- Used for handling **actions originated outside** the slice
- often used for async actions created with `createAsyncThunk`
- Could also be used when handling cross-slice actions
